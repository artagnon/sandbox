...
euler53 :: Int
euler53 = length . filter (> (10^6)) $ [div (factorial' n r) (factorial (n - r)) | n <- [1 .. 100], r <- [1 .. n]]

assertLychrel :: (Integral a) => [a] -> a -> Bool
assertLychrel x iter
    | iter == 0 = True
    | x == reverse x = False
    | otherwise = assertLychrel (extractDigits $ (numAssemble x + (numAssemble . reverse $ x))) (iter - 1)

euler55 :: Int
euler55 = length . filter (\x -> assertLychrel x 50) . map (\x -> extractDigits (x + (numAssemble . extractDigits $ x))) $ [1 .. 9999]
...
euler94 lim = takeWhile (< lim) $ [2*x + y | x <- [2 .. (div lim 3 + 1)], y <- [x - 1, x + 1], mod (4*x^2 - y^2) 4 == 0, assertTriad x y]
    ...
    where assertPerfectSquare x = x == (last . takeWhile (\y -> y <= x) $ [y^2 | y <- [1 ..]])
