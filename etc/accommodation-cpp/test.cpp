...
int main() {
  ...
  "MoreBackedges"_test = [&] {
    std::vector<std::pair<int, int>> friends{
        ...
        std::make_pair(3, 1), std::make_pair(4, 1), std::make_pair(4, 2),
        std::make_pair(4, 3)};
    assert(eq(arrangement(friends, distances),
              // Total cost: 3.2 + 11.2 + 2.4 = 16.8
              std::map<int, int>{{1, 2}, {2, 3}, {3, 1}, {4, 4}}));
  };
  ...
  "FullyConnected"_test = [&] {
    ...
    assert(eq(arrangement(friends, distances),
              // Total cost: 3.2 + 3.2 + 1.1 + 1.1 = 8.6
              std::map<int, int>{{1, 2}, {2, 1}, {3, 4}, {4, 3}}));
  };
}
